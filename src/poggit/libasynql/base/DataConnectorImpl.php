...
class DataConnectorImpl implements DataConnector {
	...
	public function loadQueryFile($fh, string $fileName = null): void {
		if (!is_resource($fh)) {
			throw new TypeError(
				"Missing $fileName in resources directory of plugin."
			);
		}
	...
	public function loadQuery(GenericStatement $stmt): void {
		if (isset($this->queries[$stmt->getName()])) {
			throw new InvalidArgumentException(
				"Duplicate GenericStatement: {$stmt->getName()}"
			);
		}
	...
	public function executeChange(
		...
		$this->executeImpl(
			...
			function (SqlChangeResult $result) use ($onSuccess) {
				if ($onSuccess !== null) {
	...
	public function executeInsert(
		...
		$this->executeImpl(
			...
			function (SqlInsertResult $result) use ($onInserted) {
				if ($onInserted !== null) {
					$onInserted(
						$result->getInsertId(),
	...
	public function executeSelect(
		...
		$this->executeImpl(
			...
			function (SqlSelectResult $result) use ($onSelect) {
				if ($onSelect !== null) {
	...
	private function executeImpl(
		...
		if (!isset($this->queries[$queryName])) {
			throw new InvalidArgumentException(
				"The query $queryName has not been loaded"
			);
		}
		$query = $this->queries[$queryName]->format(
			$args,
			$this->placeHolder,
			$outArgs
		);
	...
	private function executeImplRaw(
		string $query,
		array $args,
		int $mode,
		callable $handler,
		...
		$trace = libasynql::isPackaged()
			? null
			: new Exception(
				'(This is the original stack trace for the following error)'
			);
		$this->handlers[$queryId] = function ($result) use (
			$handler,
			$onError,
			$trace
		) {
	...
	private function reportError(
		...
		if ($error !== null) {
			...
			if ($error->getQuery() !== null) {
				$this->plugin
					->getLogger()
					->debug('Query: ' . $error->getQuery());
			}
			if ($error->getArgs() !== null) {
				$this->plugin
					->getLogger()
					->debug('Args: ' . json_encode($error->getArgs()));
			}
			if ($trace !== null) {
				$this->plugin
					->getLogger()
