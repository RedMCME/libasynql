...
class MysqlStatementImpl extends GenericStatementImpl {
	...
	protected function formatVariable(
			...
			if (empty($value)) {
				if (!$variable->canBeEmpty()) {
					throw new InvalidArgumentException(
						"Cannot pass an empty array for :{$variable->getName()}"
					);
				}
		...
		if ($value === null) {
			if (!$variable->isNullable()) {
				throw new InvalidArgumentException(
					"The variable :{$variable->getName()} is not nullable"
				);
			}
			return 'NULL';
		}
