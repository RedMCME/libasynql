...
class GenericStatementFileParser {
	...
	private function endCommand(): void {
		...
		if ($this->parsingQuery) {
			...
			$stmt = GenericStatementImpl::forDialect(
				$this->knownDialect,
				implode('.', $this->identifierStack),
				$query,
				$doc,
				$this->variables,
				$this->fileName,
	...
	private function varCommand(
		...
		try {
			$var = new GenericVariable(
				$args[0],
				$args[1],
				isset($args[2]) ? substr($line, $argOffsets[2] + 1) : null
			);
	...
	private function error(
		...
		throw new GenericStatementFileParseException(
			$problem,
			$this->lineNo,
			$this->fileName
		);
