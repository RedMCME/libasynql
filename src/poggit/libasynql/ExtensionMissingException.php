	...
	public function __construct(string $extensionName) {
		...
		if ($ini && is_file($ini)) {
			foreach (file($ini) as $i => $line) {
				if (
					strpos($line, ';extension=') !== false &&
					stripos($line, $extensionName) !== false
				) {
					$instructions =
						TextFormat::GOLD .
						'Please remove the leading semicolon on line ' .
						($i + 1) .
						" of $ini and restart the server " .
						TextFormat::RED .
						"so that the $extensionName extension can be loaded.";
				}
			}
		}

		parent::__construct(
			"The $extensionName extension is missing. $instructions"
		);
	}
}
