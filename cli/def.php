			...
			iterator_to_array(
				new RegexIterator(
					new RecursiveIteratorIterator(
						new RecursiveDirectoryIterator($argv[$i + 1])
					),
					'/\.sql$/'
				)
			)
...
fwrite(
	$fh,
	' * Created from ' . implode(', ', array_map('basename', $sqlFiles)) . $EOL
);
fwrite($fh, ' */' . $EOL);
fwrite($fh, '' . $EOL);
...
fwrite(
	$fh,
	'namespace ' . implode('\\', array_slice($fqnPieces, 0, -1)) . ';' . $EOL
);
fwrite($fh, '' . $EOL);
...
foreach ($results as $queryName => $stmts) {
	...
	implode(
		', ',
		array_map(function (GenericStatement $stmt) {
		...
		foreach ($vars0 as $varName => $var0) {
			if (isset($vars[$varName])) {
				...
				if ($var0->isOptional() !== $vars[$varName]->isOptional()) {
					echo "[*] Notice: :$varName is " .
						($var0->isOptional() ? 'optional' : 'required') .
						" for $queryName in $file0 but " .
						($var0->isOptional() ? 'required' : 'optional') .
						" in $file\n";
				}
	...
	foreach ($descLines as $line) {
		fwrite(
			$fh,
			(strlen($line) > 0 ? "{$INDENT} * $line" : "{$INDENT} *") . $EOL
		);
	}
	fwrite($fh, "{$INDENT} */" . $EOL);
	fwrite(
		$fh,
		"{$INDENT}public const {$prefix}{$const} = " .
			json_encode($queryName) .
			';' .
			$EOL
	);
	fwrite($fh, $EOL);
